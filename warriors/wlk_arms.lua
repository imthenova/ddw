function scwArmsPlay(scw, f, g)
    ...
    local debuff_zhisi = mwGetDebuffTime("致死打击");
...
--     print(speed)
    ...
    elseif cd_zhisi <= scw.cd_gcd and rage >= 30 and debuff_zhisi<=1 then
        f.textures[0]:SetColorTexture(0, 0, 0); -- 黑色致死
        g.textures[0]:SetColorTexture(0, 0, 0); -- 黑色致死
    elseif IsCurrentSpell("英勇打击") == false and rage >= 100 and (scw.tPerHealth <= 20 or buff_cs <= 0.2 or cd_zhisi > 1) then
        f.textures[0]:SetColorTexture(1, 0.8, 0.8); --7 英勇
        g.textures[0]:SetColorTexture(1, 0.8, 0.8); --7 英勇
    elseif IsCurrentSpell("英勇打击") == true and (rage <= 75) then
        f.textures[0]:SetColorTexture(0.6, 0.6, 0.6); --f11取消英勇
        g.textures[0]:SetColorTexture(0.6, 0.6, 0.6); --f11取消英勇
    ...
    elseif scw.tPerHealth <= 20 and rage>=15 then
        f.textures[0]:SetColorTexture(0.4, 0.4, 0.4); --斩杀 + 爆发
        g.textures[0]:SetColorTexture(0.4, 0.4, 0.4); --斩杀 + 爆发
    ...
    elseif cd_zhisi <= scw.cd_gcd and rage >= 30 then
        f.textures[0]:SetColorTexture(0, 0, 0); -- 黑色致死
        g.textures[0]:SetColorTexture(0, 0, 0); -- 黑色致死
    elseif IsCurrentSpell("猛击") == false and rage >= 55 and speed <= 0 then
