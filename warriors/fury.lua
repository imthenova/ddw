function dsdtFuryPlay(dstd, f, g)
    ...
    elseif isAuto and dstd.tHealth > 200000 and dstd.cd_lumang <= 0 and dstd.buff_lumang <= 0 then
        -- 鲁莽好了用鲁莽
        if dstd.cd_gcd >= 0.3 then
    ...
    elseif isAuto and dstd.buff_jn > 0.1 and dstd.cd_spear <= dstd.cd_gcd then
        g.textures[0]:SetColorTexture(0, 0, 1) --蓝 巨龙 e&.
    elseif dstd.buff_jn > 0.1 and dstd.cd_panzui <= dstd.cd_gcd and (isZhanShaPeriod or dstd.buff_cs > 0.5) then
        -- 斩杀阶段有激怒
